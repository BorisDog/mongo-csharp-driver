...
namespace MongoDB.Driver.Tests.Specifications.retryable_reads.prose_tests
{
    public class PoolClearedTests
    {
        [SkippableTheory]
        [ParameterAttributeData]
        public void Should_retry_after_PoolClearedError_on_read([Values(false, true)] bool async)
        {
            RequireServer.Check().Supports(Feature.RetryableReads, Feature.FailPointsBlockConnection);
            ...
            ThreadingUtilities.ExecuteOnNewThreads(2, i =>
            {
                ...
                {
                    _ = collection
                        .FindAsync(new BsonDocument()).GetAwaiter().GetResult()
