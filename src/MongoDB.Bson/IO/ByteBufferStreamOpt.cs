...
namespace MongoDB.Bson.IO
{
    ...
    /// </summary>
    public class ByteBufferStreamOpt : BsonStream, IStreamEfficientCopyTo
    {
        // private fields
        ...
        private int _currentBufferMaxPosition = 0;
        private int _currentBufferOffset = int.MaxValue;
        ...
        public ByteBufferStreamOpt(IByteBuffer buffer, bool ownsBuffer = false)
        {
            if (buffer == null)
            {
        ...
        public override int ReadByte()
        {
            ...
            var countLeft = _currentBufferMaxPosition - _position;
            if (countLeft > 0)
            {
                return _currentBuffer[_currentBufferOffset + _position++];
            }
            else
            {
        ...
        public override unsafe bool ReadCStringBytesAndCompare(byte[] bytesToCompare)
        {
            var segment = GetSegment();

            int index = -1;
            var compareLength = bytesToCompare.Length;
            if (compareLength < segment.Count)
            {
                fixed (byte* pSrc = &segment.Array[segment.Offset])
                fixed (byte* pDst = &bytesToCompare[0])
                {
                    if (pSrc[compareLength] == 0 &&
                        memcmp(new IntPtr(pSrc), new IntPtr(pDst), new IntPtr(compareLength)) == 0)
                    {
                        _position += compareLength + 1;
                        return true;
                    }
                }
            }
            ...
            {
                this.ReadBytes(_temp, 0, compareLength + 1);

                fixed (byte* pSrc = &_temp[0])
                fixed (byte* pDst = &bytesToCompare[0])
                {
                    if (pSrc[compareLength] == 0 &&
                        memcmp(new IntPtr(pSrc), new IntPtr(pDst), new IntPtr(compareLength)) == 0)
        ...
        /// <inheritdoc/>
        public unsafe override ArraySegment<byte> ReadCStringBytes()
        {
            //ThrowIfDisposed();
            ...
            fixed (byte* p = &segment.Array[segment.Offset])
            {
                ...
                var maxLong = segment.Count >> 3;
                var checkedCount = maxLong << 3;

                for (int i = 0; i < maxLong; i++)
                {
                    var v = *longP++;
                    var z = (v - 0x0101010101010101) & ~(v) & 0x8080808080808080;
                ...
                var byteP = p + checkedCount;
                for (int i = checkedCount; i < segment.Count; i++, byteP++)
                {
                    if (*byteP == 0)
                    {
                        index = i + segment.Offset;
                        break;
                    }
                }
            }

            //var index2 = Array.IndexOf<byte>(segment.Array, 0, segment.Offset, segment.Count);
            //if (index2 != index)
            //{
            //    throw new Exception("");
            //}
        ...
        /// <inheritdoc/>
        unsafe public override int ReadInt32()
        {
            //ThrowIfDisposed();
        ...
        {
            //return _buffer.AccessBackingBytes(_position);

            var countLeft = _currentBufferMaxPosition - _position;
            if (countLeft > 0)
            {
                return new ArraySegment<byte>(_currentBuffer, _currentBufferOffset + _position, countLeft);
            }

            var segment = _buffer.AccessBackingBytes(_position);
            _currentBuffer = segment.Array;
            _currentBufferMaxPosition = _position + segment.Count;
            _currentBufferOffset = segment.Offset - _position;
