...
namespace MongoDB.Bson.IO
{
    ...
    /// </summary>
    public sealed class MultiChunkBufferOpt : IByteBuffer
    {
        // private fields
        ...
        public MultiChunkBufferOpt(IBsonChunkSource chunkSource)
        {
            if (chunkSource == null)
            {
        ...
        public MultiChunkBufferOpt(IEnumerable<IBsonChunk> chunks, int? length = null, bool isReadOnly = false)
        {
            if (chunks == null)
            {
        ...
        public ArraySegment<byte> AccessBackingBytes(int position)
        {
            if (_chunkLength > 0)
            {
                var chunkIndexNew = position >> 16;

                if (chunkIndexNew < _chunks.Count)
                {
                    var segment = _chunks[chunkIndexNew].Bytes;
                    var chunkOffset = position - (chunkIndexNew << 16);
                    var chunkRemaining = _chunkLength - chunkOffset;
                ...
                else
                {
                    if (_chunks.Count > 0)
                    {
                        var segment = _chunks[chunkIndexNew - 1].Bytes;
        ...
        public IByteBuffer GetSlice(int position, int length)
        {
            ...
            else
            {
                ...
                forkedBuffer = new MultiChunkBufferOpt(forkedChunks, forkedBufferLength, isReadOnly: true);
            }

            var offset = position - _positions[firstChunkIndex];
            return new ByteBufferSliceOpt(forkedBuffer, offset, length);
        }
